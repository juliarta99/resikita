<div class="space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-400">{{ $report->tiket_no }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div class="space-y-5 lg:col-span-2">
            <div class="rounded-xl border border-slate-200 bg-white p-5">
                ...
                @if ($report->foto)
                    <img src="{{ asset('storage/'.$report->foto) }}" class="mt-4 max-h-72 w-full rounded-lg object-cover">
                @endif

                @if ($report->lat && $report->lng)
                    <a href="https://www.google.com/maps/search/?api=1&query={{ $report->lat }},{{ $report->lng }}" target="_blank"
                       class="mt-4 inline-flex items-center gap-2 rounded-lg border border-emerald-600 px-3 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5">
                ...
                @forelse ($timeline as $p)
                    <div class="flex gap-3 border-l-2 border-emerald-100 pb-5 pl-4 last:pb-0">
                        <div class="-ml-[22px] mt-0.5 flex h-4 w-4 items-center justify-center rounded-full {{ $p->status_progress === 'selesai' ? 'bg-emerald-600' : 'bg-blue-500' }}">
                        </div>
                        <div class="flex-1">
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-6 rounded-xl border border-slate-200 bg-white p-5">
                @if ($sudahSelesai)
