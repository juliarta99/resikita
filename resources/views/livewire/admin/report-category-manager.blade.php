<div class="mx-auto max-w-3xl space-y-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-primary-900">Kategori Laporan</h1>
            <p class="mt-1 text-sm text-gray-500">Kategori untuk pelaporan masalah sampah oleh masyarakat.</p>
        </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($daftar as $c)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-3 text-gray-600">{{ $c->deskripsi ?? '—' }}</td>
                ...
                @empty
                    <tr><td colspan="3" class="px-6 py-8 text-center text-gray-400">Belum ada kategori.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <x-modal active="showForm" max-width="max-w-lg">
        <form wire:submit="simpan">
            ...
            <div class="space-y-4 px-6 py-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-primary-900">Deskripsi (opsional)</label>
                    <input wire:model="deskripsi" class="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-primary-900 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
                </div>
            </div>
    ...
    <x-confirm active="showDelete" action="hapus" title="Hapus kategori?"
               message="Kategori laporan ini akan dihapus permanen." />
</div>