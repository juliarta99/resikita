<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div>
            ...
            <input type="date" wire:model.live="dari" class="mt-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
        <div>
            <label class="block text-xs font-medium text-gray-500">Sampai</label>
            <input type="date" wire:model.live="sampai" class="mt-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
    ...
    <div class="grid gap-4 sm:grid-cols-3 lg:grid-cols-6">
        @foreach ([
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <p class="mt-1 text-xl font-semibold {{ $cls }}">{{ $val }}</p>
            </div>
        @endforeach
    </div>
                ...
                @forelse ($perKategori as $k)
                    ...
                    <div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-primary-900">{{ $k['nama'] }}</span>
                            <span class="text-gray-500">{{ $k['selesai'] }}/{{ $k['total'] }} selesai</span>
                        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="text-xs uppercase tracking-wider text-gray-400"><tr><th class="py-2 font-semibold">Petugas</th><th class="py-2 text-right font-semibold">Ditugaskan</th><th class="py-2 text-right font-semibold">Selesai</th></tr></thead>
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($perPetugas as $p)
                            <tr><td class="py-2 text-primary-900">{{ $p['nama'] }}</td><td class="py-2 text-right text-gray-600">{{ $p['ditugaskan'] }}</td><td class="py-2 text-right font-medium text-primary-700">{{ $p['selesai'] }}</td></tr>
                        @empty
