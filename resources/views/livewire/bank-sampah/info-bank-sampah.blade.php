<div class="mx-auto max-w-2xl space-y-6">
    ...
    <form wire:submit="simpan" class="space-y-5 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div>
            ...
            <div class="mt-2 flex items-center gap-4">
                @if ($foto && method_exists($foto, 'temporaryUrl'))
                    <img src="{{ $foto->temporaryUrl() }}" class="h-24 w-24 rounded-lg border border-gray-200 object-cover">
                @elseif ($fotoLama)
                    <img src="{{ asset('storage/' . $fotoLama) }}" class="h-24 w-24 rounded-lg border border-gray-200 object-cover">
                @else
                    <div class="flex h-24 w-24 items-center justify-center rounded-lg border border-dashed border-gray-300 text-xs text-gray-400">Tanpa foto</div>
                @endif
        ...
        <div class="flex justify-end">
            <button type="submit" class="rounded-lg bg-primary-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-700">Simpan Perubahan</button>
        </div>
    </form>
</div>